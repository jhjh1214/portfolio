...
interface Props { pcb: string; accent: string; onBoot: () => void; still: boolean }

/** A stylised ESP32 dev board. Drag to rotate; press the BOOT button. */
function Board({ pcb, accent, onBoot }: Omit<Props, 'still'>) {
  ...
  useFrame(({ clock }) => {
    if (led.current) led.current.emissiveIntensity = on ? 1.6 + Math.sin(clock.elapsedTime * 9) * 1.2 : 0.15
  ...
  return (
    <group rotation={[0.1, -0.55, 0]} scale={1.05}>
      <mesh>
        ...
        <meshStandardMaterial color={pcb} roughness={0.55} metalness={0.15} />
      </mesh>
      {/* WROOM module */}
      ...
      {[0, 1, 2, 3].map((i) => (
        <mesh key={i} position={[0.55 + i * 0.16, 0.09, i % 2 ? -0.28 : 0.28]}>
          ...
          <meshStandardMaterial color="#d9b44a" metalness={0.8} roughness={0.35} />
        </mesh>
      ))}
      {/* header pins */}
...
export default function BoardScene({ pcb, accent, onBoot, still }: Props) {
  return (
    <Canvas dpr={[1, 1.75]} camera={{ position: [0, 3.4, 4.6], fov: 34 }} frameloop={still ? 'demand' : 'always'} gl={{ alpha: true, antialias: true }}>
      ...
      <PresentationControls global={false} snap speed={1.6} polar={[-0.5, 0.5]} azimuth={[-0.9, 0.9]}>
        <Board pcb={pcb} accent={accent} onBoot={onBoot} />
